...
class VietinbankParser(BaseStatementParser):
  # static values 
  REGEXES = {
    "Date": r"^\d{2}\/\d{2}\/\d{4}$",
  ...
  COLUMN_DIVIDERS_IN_PIXEL = [0, 374, 624, 1354, 1664, DEFAULT_WIDTH_PIXEL] # convert pdf to image and measure this

  def __init__(self, input_file, output_folder, structure, mode, fromOutputFileId: int):
    if structure != 'tabular':
      raise ValueError(f"Structure '{self.structure}' is not supported for Vietinbank. Only 'tabular' is supported.")
  ...
  def textLocationBasedExtraction(self, page, numItemsInTheFirstColumn = 0):
    ...
    for (j, word) in sortedWords:
      if re.match(VietinbankParser.REGEXES["Date"], word['text'].strip()):
    ...
    for i in range(1, len(row_top_positions)):
      ...
      colDividers = VietinbankParser.COLUMN_DIVIDERS_IN_PIXEL
      for col in range(1, len(colDividers)):
        ...
        topPixel = top/page.height * VietinbankParser.DEFAULT_HEIGHT_PIXEL
        bottomPixel = bottom/page.height * VietinbankParser.DEFAULT_HEIGHT_PIXEL
  ...
  def parseTextPdf(self):
    ...
    print(f"Parsing Vietinbank text PDF file: {self.input_file}")
    (countPrevTransactions, lastPageId) = self.getFirst1BasedPositionsToParse()
    start_row_id = VietinbankParser.NUM_TRANSACTIONS_PER_FILE * (self.fromOutputFileId - 1) + 1
  ...
  def parseImagePdf(self):
    # VietinbankParser does not support imagePdf mode
    raise NotImplementedError("VietinbankParser does not support imagePdf mode.")